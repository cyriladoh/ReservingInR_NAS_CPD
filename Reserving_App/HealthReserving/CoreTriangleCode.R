...
reserve_df <- function(x, ocr, ep, ielr_s, f, selected_mthd, format = F){
  ...
  res_df <- res_df %>% mutate(f_inverse =1/f, earned_premium = 1000, ielr = ielr_s, CL_Utl = round(paid_to_date*f))
  ...
  res_df <- res_df %>% mutate(selected_method = selected_mthd, CL_ibnr = round(CL_Utl - incurred_claim), BF_ibnr = round(BF_Utl - incurred_claim), LR_ibnr = round(LR_Ult - incurred_claim), selected_ibnr = ifelse(selected_method == "CL", CL_ibnr, ifelse(selected_method == "BF", BF_ibnr, LR_ibnr)))
